...
#if defined(NDEBUG)
# error "Empirejadecoin cannot be compiled without assertions."
#endif
...
CriticalSection cs_main;

CTxMemPool mempool;
unsigned int nTransactionsUpdated = 0;

map<uint256, CBlockIndex*> mapBlockIndex;
uint256 hashGenesisBlock("0xe0b65321c304156509ed165d8c4fa12d2dcadf9f84f7951043d32206a1f97e28");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Empirejadecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Empirejadecoin Signed Message:\n";
...
int64 nMinimumInputValue = DUST_HARD_LIMITS;
...
{
    // Empirejadecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 10 * 365 * 24 * 60 * 60; // Empirejadecoin: 10 years
static const int64 nTargetSpacing = 1 * 60; // Empirejadecoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Empirejadecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Empirejadecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe0b65321c304156509ed165d8c4fa12d2dcadf9f84f7951043d32206a1f97e28");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 26/Jun/2016 Election in Spain Gives Conservatives More Seats but No Mandate";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049898610fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1466948073;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1404422;
        ...
        assert(block.hashMerkleRoot == uint256("0x4f61702e0a6eef0ad5264754f4d5c54888f475aa4858c3690a9f78e29d4dd54a"));
...
unsigned char pchMessageStart[4] = { 0xbb, 0xd6, 0xb6, 0xbd }; // Empirejadecoin: increase each by adding 2 to bitcoin's value.
            ...
            for (unsigned int i = 0; i < vWorkQueue.size(); i++)
            {
                     ...
                     ++mi)
                {
                    ...
                    else if (!fMissingInputs2)
                    {
                        ...
                        vEraseQueue.push.back(orphanHash);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EmpirejadecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EmpirejadecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EmpirejadecoinMiner(CWallet *pwallet)
{
    printf("EmpirejadecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("empirejadecoin-miner");
    ...
    try { loop {
        ...
        printf("Running EmpirejadecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EmpirejadecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EmpirejadecoinMiner, pwallet));
}
